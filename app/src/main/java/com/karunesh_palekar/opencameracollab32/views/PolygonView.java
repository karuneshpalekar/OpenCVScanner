...
public class PolygonView extends FrameLayout {
    ...
    //Send to the bitmap as the final points as normal or as set by the users after edge detection
    public List<PointF> getsPoints() {
        ...
        return getOrderedPointss(points);
    }

    public List<PointF> getOrderedPointss(List<PointF> points) {

        List<PointF> pointss = new ArrayList<>();
        PointF centerPoint = new PointF();
        int size = points.size();
        for (PointF pointF : points) {
            centerPoint.x += pointF.x / size;
        ...
        for (PointF pointF : points) {
            ...
            if (pointF.x < centerPoint.x && pointF.y < centerPoint.y) {
               index = 0 ;
                pointss.add(new PointF(pointF.x,pointF.y));
            } else if (pointF.x > centerPoint.x && pointF.y < centerPoint.y) {
                index = 1;
                pointss.add(new PointF(pointF.x,pointF.y));
            } else if (pointF.x < centerPoint.x && pointF.y > centerPoint.y) {
    ...
    //Getting the points from the Image after finding the contours
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        //Point - the midpoint = gives the center
